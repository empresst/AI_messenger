...
# # ---------------------
# # Ensure NLTK data goes to a writable dir on Render
...
# import spacy
# try:
#     # Try to load the small English model if present
#     nlp = spacy.load("en_core_web_sm")
# except Exception:
#     # Fallback so the app still boots even if the model fails to install
...
# # Optional: only seed demo data if you explicitly set SEED_DEMO=true
...
#                 MONGODB_URI,
#                 tls=True,
#                 tlsAllowInvalidCertificates=True,  # works fine with Atlas SRV + Render egress
#                 maxPoolSize=50,
#                 minPoolSize=5,
#                 maxIdleTimeMS=30000,
#                 tz_aware=True,
...
# # ---------------------
# # Healthcheck (important for Render)
# # ---------------------
# @app.get("/healthz")
# async def healthz():
#     return {"ok": True}
...
#     </div>
#   `;
#   document.getElementById('chatGrid').appendChild(div);
#   const area = document.getElementById('inp_'+u.user_id);
#   const btn = document.getElementById('send_'+u.user_id);
#   const typingEl = document.getElementById('typing_'+u.user_id);
...
#   const pane = document.getElementById('msg_'+u.user_id);
#   pane.innerHTML='';
...
#   const pane = document.getElementById('msg_'+other_id);
#   if(!pane) return;
...
#   const elT = document.getElementById('typing_'+other_id);
#   if(!elT) return;
...
#   document.getElementById('gatewayKey').value = localStorage.getItem('gw') || '';
#   API_KEY = document.getElementById('gatewayKey').value;
#   document.getElementById('gatewayKey').addEventListener('change', ()=>{ API_KEY = document.getElementById('gatewayKey').value; localStorage.setItem('gw', API_KEY) });
...
#     const display_name = document.getElementById('signupDisplayName').value.trim() || username;
...
#     const other = document.getElementById('relOther').value;
#     const relation = document.getElementById('relKind').value;
#     if(!other) return;
#     await req('/relationships/set','POST', {other_user_id: other, relation});
#     document.getElementById('relStatus').innerText = 'Saved!';
#     setTimeout(()=>{document.getElementById('relStatus').innerText=''},1500);
#     await refreshUsers();
#   };
...
#     try:
#         doc = nlp(user_input)
#         # If using spacy.blank, tokens may not have POS/stop flags; fallback to simple tokens
#         key_terms = []
#         for t in doc:
...
#             FAISS.from_texts  # keep linter happy
...
# # ---------------------
# # Demo seed / initialization (guarded)
# # ---------------------
...
#     # In production, DO NOT nuke your DB.
#     # Only seed when SEED_DEMO=true (e.g., for local dev or a one-time demo)
#     if SEED_DEMO:
...
#         await verify_data()
#     # Always ensure FAISS is ready (rebuilds from embeddings if needed)
#     await initialize_faiss_store()
...
#     import uvicorn
#     # Respect Render's $PORT, proxy headers, and keep WS alive a bit longer
#     uvicorn.run(
#         app,
#         host="0.0.0.0",
#         port=PORT,
