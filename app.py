...
import asyncio, numpy as np, pytz
...
async def should_include_memories(user_input: str, speaker_id: str, user_id: str) -> Tuple[bool, List[dict]]:
    ...
    return (len(relevant_memories) > 0), relevant_memories[:3]
...
async def initialize_bot(
    ...
    # Auto-resolve role if not given or unhelpful
    role_in = (bot_role or "").strip().lower()
    if not role_in or role_in == "friend":
        role_in = await resolve_target_role_for_reply(speaker_id, target_id)
    ...
    history_for_prompt = recent[:]
    if recent and recent[-1].get("content", "").strip() == user_input.strip():
        history_for_prompt = recent[:-1]

    hist_text = "\n".join([f"{m['speaker']}: {m['content']}" for m in history_for_prompt]) or "No recent conversation history."
    last_ts = history_for_prompt[-1]["raw_timestamp"] if history_for_prompt else None
    use_greeting = (not history_for_prompt) or (datetime.now(pytz.UTC) - as_utc_aware(last_ts)).total_seconds() / 60 > 30
    greeting, tone = await get_greeting_and_tone(role_in, target_id)
    ...
    if include and mems:
        valid_memories = [
            m for m in mems if all(k in m for k in ["content", "type", "timestamp", "speaker_name"])
        ]
        if valid_memories:
            mems_text = "\n".join([
    ...
    # Build prompt using first code’s detailed style
    sp_name = (sp or {}).get("display_name") or (sp or {}).get("username") or speaker_id
    tg_name = (tg or {}).get("display_name") or (tg or {}).get("username") or target_id
    trait_str = ', '.join([f"{k} ({v['explanation']})" for k, v in list(traits.get('core_traits', {}).items())[:3]]) or "balanced"
    ...
    if include:
        prompt = f"""
        You are {tg_name}, responding as an AI Twin to {sp_name}, you are his/her {role_in}.
        Generate a short 2-3 sentence reply that:
        - Uses a {tone} tone, appropriate for your relationship with {sp_name}.
        - Reflects your personality: {trait_str}.
        ...
        prompt = f"""
        You are {tg_name}, responding as an AI Twin to {sp_name}, his/her {role_in}.
        Generate a short 2-3 sentence reply that:
        - Uses a {tone} tone, appropriate for your relationship with {sp_name}.
        - Reflects your personality: {trait_str}.
    ...
    return prompt, greeting, use_greeting, traits
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            # --- Save user message ---
            user_doc = await save_and_embed_message(user_id, to, text, source="human")

            # --- Broadcast user message to recipient ---
            await manager.send_to(to, {
                "type": "chat",
                "from": user_id,
                "payload": {
                    "speaker_id": user_doc["speaker_id"],
                    "target_id": user_doc["target_id"],
            ...
            # --- Generate AI response if target has AI enabled ---
            tgt = await users_col.find_one({"user_id": to})
            if tgt and tgt.get("ai_enabled", False):
                try:
                    prompt, greeting, use_greeting, traits = await initialize_bot(user_id, to, None, text)
                    ai_text = await generate_response(
                        prompt, text, greeting, use_greeting, user_id, to, None, traits
                    )

                    # --- Save AI response ---
                    ai_doc = await save_and_embed_message(to, user_id, ai_text, source="ai_twin")
                    ...
                    await manager.send_to(user_id, {
                        ...
                        "from": to,
                        "payload": {
                            "speaker_id": ai_doc["speaker_id"],
                            "target_id": ai_doc["target_id"],
